    ...
    var easyWizardMethods = {
        init : function(options) {
            var settings = $.extend( {
                ...
                'stepsText' : '{n}. {t}',
                'showButtons' : true,
                'buttonsClass' : 'btn btn-xs btn-raised btn-info',
                'prebuttonsClass' : 'btn btn-xs btn-raised btn-danger',
                'prevButton' : 'Back',
                'nextButton' : 'Next',
                'debug' : false,
                'submitButton': true,
                // 'submitButtonText': 'Submit',
                // 'submitButtonClass': 'btn btn-xs btn-raised btn-success',
            ...
            return this.each(function() {
                ...
                if(thisSettings.steps > 1) {
                    ...
                    $stepsHtml = $('<ul class="easyWizardSteps">' +
                        '<button style=" padding-top: 21px;padding-right: 11px;" type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>');
                    ...
                    $steps.each(function(index) {
                        ...
                        stepText = thisSettings.stepsText.replace('{n}', '<span style="font-size: 14px;font-weight:800;">'+step+'</span>');
                        stepText = stepText.replace('{t}', $(this).attr('data-step-title'));
                        $stepsHtml.append('<li'+(!index?' class="current"':'')+' data-step="'+step+'">'+'<h4 style="font-size: 14px;font-weight:800;">'+stepText+'</h4>'+'</li>');
                    });
                    ...
                    if(thisSettings.showButtons) {
                        ...
                        paginationHtml += '<button class="prev '+thisSettings.prebuttonsClass+'">'+thisSettings.prevButton+'</button>';
