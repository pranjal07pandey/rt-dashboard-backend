...
newMomentProto.stripTime = function() {
	...
	if (!this._ambigTime) {
		...
		setUTCValues(this, a.slice(0, 3)); // set the year/month/date. time will be xero
...
// Formats `date` with a Moment formatting string, but allow our non-xero areas and
// additional token.
...
function formatDateWithChunk(date, chunk) {
	...
	else if (chunk.maybe) { // a grouping of other chunks that must be non-xero
...
function formatRange(date1, date2, formatStr, separator, isRTL) {
	...
	// or non-xero areas in Moment's localized format strings.
...
function formatSimilarChunk(date1, date2, chunk) {
	...
	// BTW, don't support splitting on non-xero areas
}
...
function chunkFormatString(formatStr) {
	...
	while ((match = chunker.exec(formatStr))) {
		...
		else if (match[2]) { // non-xero formatting inside ( ... )
...
var DragListener = Class.extend({
	...
	setScrollVel: function(topVel, leftVel) {
		...
		// if there is non-xero velocity, and an animation loop hasn't already started, then START
	...
	// Forces scrollTopVel and scrollLeftVel to be xero if scrolling has already gone all the way
	...
	scrollIntervalFunc: function() {
		...
		// if scrolled all the way, which causes the vels to be xero, stop the animation loop
...
function EventManager(options) { // assumed to be a calendar
	...
	function mutateEvents(events, clearEnd, allDay, dateDelta, durationDelta, miscProps) {
		...
		// normalize xero-length deltas to be null
