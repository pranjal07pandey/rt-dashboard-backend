                    ...
                    <div class="row invoice-info">
                        <div class="col-md-4 invoice-col">
                            @if(AmazoneBucket::fileExist(@$sentDocket->senderCompanyInfo->logo))
                                <img src="{{ AmazoneBucket::url() }}{{ @$sentDocket->senderCompanyInfo->logo }}" style="height:150px;">
                            @else
                        ...
                        <div class="pull-right" style="text-align:left;width:170px;">
                            ...
                            <b>Docket ID:</b> e-doc {{ @$sentDocket->id }}<br>
                        </div>
                        <!-- /.col -->
                    </div>
                    <div class="row">
                            ...
                            <table class="table table-striped">
                                ...
                                @if($docketFields)
                                    ...
                                    @foreach($docketFields as $row)
                                                            ...
                                                            @foreach($row->sentDocketImageValue as $signature)
                                                                <li style="margin-right:10px;float: left;">
                                                                    <a href="{{ AmazoneBucket::url() }}{{ $signature->value }}" target="_blank">
                                                                        <img src="{{ AmazoneBucket::url() }}{{ $signature->value }}" width="100px">
                                                                    </a>
                                                                </li>
                                                            @endforeach
                                                        </ul>
                                        ...
                                        @elseif($row->docketFieldInfo->docket_field_category_id!=13 && $row->docketFieldInfo->docket_field_category_id!=17)
                                            <tr>
                                                <td> {{ $row->label }}</td>
                                                <td> {{ $row->value }}</td>
                                            </tr>
                                    ...
                                    @foreach($docketFields as $row)
                                        @if($row->docketFieldInfo->docket_field_category_id==13)
                                            <tr>
                                                <td  colspan="2"> <strong style="font-size: 14px; font-weight: 300;"><b>{{ $row->label }}</b></strong><br>
                                                    {{ $row->value }}
                                                </td>
                                            </tr>
                                        @endif
...
//                        $("#mobileviewHtml").html(response);
                    ...
                    $.ajax({
                        type: "GET",
                        url: "{{url('dashboard/company/docketBookManager/docket/approvalTypeView/'.$sentDockets->id) }}",
                        success: function (response) {
                            $("#mobileviewHtml").html(response);
                        }
                    });
