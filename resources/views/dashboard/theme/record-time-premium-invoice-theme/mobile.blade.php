            ...
            <div id="printContainer">
                ...
                <div class="division-one">
                    <div class="invoice-from">
                        @if(AmazoneBucket::fileExist(@$invoice->senderCompanyInfo->logo))
                            <img style="width:100px;" src="{{ AmazoneBucket::url() }}{{ @$invoice->senderCompanyInfo->logo }}">
                        @else
                            <img src="https://dummyimage.com/100x100/f0f0f0/ffffff.jpg&text=your+logo">
                        @endif
                        <ul>
                            <li class="deam" style="font-size:12px; margin: 0px;">From:</li>
                            <li class="bold-d" style="font-size:12px; margin: 0px;">{{ @$invoice->senderUserInfo->first_name }} {{ @$invoice->senderUserInfo->last_name }}</li>
                            <li style="font-size:12px; margin: 0px;">{{ @$invoice->senderUserInfo->email }}</li>
                        </ul>
                    </div>
                    <div class="division-logo">
                        <ul>
                            <li class="bold-d" style="font-size:12px; margin: 0px;" >{{ @$invoice->senderCompanyInfo->name }}</li>
                            <li class="deam" style="font-size:12px; margin: 0px;">{{ @$invoice->senderCompanyInfo->address }}</li>
                            <li class="deam" style="font-size:12px; margin: 0px;">ABN: {{ @$invoice->senderCompanyInfo->abn }}</li>
                            <li class="deam" style="font-size:12px; margin: 0px;">Phone: &nbsp;{{ @$invoice->senderCompanyInfo->contactNumber }}</li>
                        </ul>
                    </div>
                </div>
                <div class="division-o">
                    <div class="invoiceto">
                        <div class="invoice-to">
                            <ul>
                                <li class="deam" style="font-size:12px; margin: 0px;">To:</li>
                                <li class="bold-d" style="font-size:12px; margin: 0px;">{{ $invoice->receiverUserInfo->first_name }} {{ $invoice->receiverUserInfo->last_name }}</li>
                                <li style="font-size:12px; margin: 0px;">{{ @$invoice->receiverCompanyInfo->name }}</li>
                                <li style="font-size:12px; margin: 0px;">{{ @$invoice->receiverCompanyInfo->address }}</li>
                            </ul>
                        </div>
                    </div>
                    <div class="invoice-tax">
                        <div class="tax-invoice">
                            <p style="font-size:12px;" class="bold-d tax-bold">TAX INVOICE</p>
                            <div class="invoice-date">
                                <ul style="padding-bottom:3px;">
                                    <li><b style="font-size:12px; margin: 0px;">Invoice ID:</b><span style="font-size:11px; margin: 0px;">{{ $invoice->id }}</span></li>
                                    <li><b style="font-size:12px; margin: 0px;">Date:</b><span style="font-size:11px; margin: 0px;">{{ \Carbon\Carbon::parse($invoice->created_at)->format('d-M-Y') }}</span></li>
                                </ul>
                            </div>
                        </div>
                    </div>
                ...
                <div class="amount-value">
                    <table cellpadding="0" class="responsive-table">
                        <thead>
                        <tr class="">
                            <th style="font-size:12px;" class="amount-dess">DESCRIPTION</th>
                            <th style="font-size:12px;" class="value-dess">VALUE/ AMOUNT</th>
                        </tr>
                        </thead>
                        <tbody>
                        <?php $i=1 ?>
                        @if($sentInvoiceValue)
                            @foreach($sentInvoiceValue as $item)
                                @if($item["invoice_field_category_id"]!=9 && $item["invoice_field_category_id"]!=12 && $item["invoice_field_category_id"]!=5)
                                    <tr>
                                    <td style="font-size:12px;" class="bold-d padding-oops"> {{ $item["label"] }}</td>
                                    <td style="font-size:12px;" class="bold-d">{{ $item["value"] }}</td>
                                    </tr>
                                @endif
                            @endforeach
                        @endif
                            ...
                            @if($invoice->attachedDocketsInfo->count()>0)
                                @foreach($invoice->attachedDocketsInfo as $invoiceDocket)
                                    <tr>
                                        <td>
                                            <ul>
                                                <li class="bold-d" style="font-size:12px;"> {{ $invoiceDocket->docketInfo->docketInfo->title }}</li>
                                                <li style="font-size:12px;" class="deam">Docket: #Doc{{ $invoiceDocket->docketInfo->id }}</li>
                                                <li style="font-size:12px;" class="deam">
                                        ...
                                        <td style="font-size:12px;" class="bold-d ">
                                            <?php
                                            $invoiceAmount  =    0;
                ...
                <div class="payment-method">
                        ...
                        <table class="right-table" style="font-size:12px;">
                            <tbody>
                            <tr>
                                <td>Sub Total</td>
                                <td class="total-right bold-d">$ {{ round($invoice->amount,2) }}</td>
                            </tr>
                            <tr>
                                <td>{{ $invoice->invoiceInfo->gst_label }}</td>
                                <td class="total-right bold-d">$ {{ round($invoice->amount*$invoice->gst/100,2)  }}</td>
                            </tr>
                            <tr class="grand-total">
                                <td class="bold-d">GRAND TOTAL</td>
                                <td class="total-right bold-d">
                                    @if($invoice->gst!= 0)
                                            ...
                                            <ul class="signature-list">
                                                @foreach($images as $signature)
                                                    <li>
                                                        ...
                                                        <img src="{{ AmazoneBucket::url() }}{{ $signature->value }}">
                                                        </a>
                                                        </li>
                    ...
                    <div class="payment-left">
                        @if($invoiceSetting)
                            <table class="payment-table" style="font-size:12px; line-height:18px; margin:0;">
                                <thead>
                                <tr class="payment-title">
                                    <td style="font-size:12px;" colspan="2"> PAYMENT METHOD</td>
                                </tr>
                                </thead>
                                ...
                                <tr>
                                    <td colspan="2">
                                        <ul class="important-info important-height">
                                            {{--  <li class="bold-d yes-imp"> IMPORTANT INFORMATION</li>
                                             <li class="bold-d">Quote Invoice number as reference when making deposits</li>
                                        ...
                                        @if($images->count()>0)
                                            <ul>
                                                @foreach($images as $image)
                                                    <li>
                                                                                                        <a href="{{ AmazoneBucket::url() }}{{ $signature->value }}" target="_blank">
                                                        <img src="{{ AmazoneBucket::url() }}{{ $image->value }}">
                                                          </a>
                                                        </li>
                        ...
                        <!--division-one-e  -->
                            <div class="division-one-e">
                                ...
                                <div class="invoice-fromm">
                                    @if(AmazoneBucket::fileExistt::fileExistt::fileExist(@$invoice->senderCompanyInfo->logo))
                                        <img style="width:100px;" src="{{ AmazoneBucket::url() }}{{ @$invoice->senderCompanyInfo->logo }}">
                                    @else
                                        <img src="https://dummyimage.com/100x100/f0f0f0/ffffff.jpg&text=your+logo">
                                    @endif
                                ...
                                <div class="division-logoo" style="font-size:12px;">
                                    <ul>
                                        <li class="bold-d">{{  $row->docketInfo->senderCompanyInfo->name  }}</li>
                                        <li class="deam">{{  $row->docketInfo->senderCompanyInfo->address  }}</li>
                                        <li class="deam">ABN:{{  $row->docketInfo->senderCompanyInfo->abn  }}</li>
                                        <li class="deam">Phone:&nbsp;{{  $row->docketInfo->senderCompanyInfo->contactNumber  }}</li>
                                    </ul>
                                </div>
                            </div>
                            <div class="division-o">
                                <div class="invoiceto">
                                    <div class="invoice-to" style="font-size:12px;">
                                        <ul>
                                            <li class="deam">To:</li>
                                            @if($row->docketInfo->recipientInfo)
                                                ...
                                                @endforeach
                                            @endif
                                            {{-- <li>No Paiso PTY LTD.</li>
                                            <li>Level 5,7 London Circuit,ACT,2601</li> --}}
                                        </ul>
                                    </div>
                                </div>
                                <!--<div class="invoice-tax" >-->
                                <!--    <div class="tax-invoice" style="font-size:12px;">-->
                                <!--        <p class="tax-bold">{{ $row->docketInfo->title }}</p>-->
                                <!--        <div class="invoice-date">-->
                                <!--            <ul>-->
                                ...
                                <!--    </div>-->
                                <!--</div>-->
                                  <div class="invoice-tax" style="margin-top:-18px;">
                                    <div class="tax-invoice">
                                        <div class="invoice-date">
                                            <ul style="padding-bottom:3px;">
                                                <li><b style="font-size:12px; margin: 0px;">Invoice ID:</b><span style="font-size:11px; margin: 0px;">{{ $row->docketInfo->id }}</span></li>
                                                <li><b style="font-size:12px; margin: 0px;">Date:</b><span style="font-size:11px; margin: 0px;">{{ \Carbon\Carbon::parse($row->docketInfo->created_at)->format('d-M-Y') }}</span></li>
                                            </ul>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <!-- description value/amount -->
                            <div class=" amount-valuedocsix">
                                <table cellpadding="0" class="responsive-tab" style="font-size:12px;">
                                    <thead>
                                    <tr class="">
                                    ...
                                    @if($row->docketInfo->sentDocketValue)
		          		        	...
		          		        	@if($item->docketFieldInfo->docket_field_category_id!=13 && $item->docketFieldInfo->docket_field_category_id!=12  && $item->docketFieldInfo->docket_field_category_id!=15 && $item->docketFieldInfo->docket_field_category_id!=9 && $item->docketFieldInfo->docket_field_category_id!=5 && $item->docketFieldInfo->docket_field_category_id!=14  && $item->docketFieldInfo->docket_field_category_id!=8 && $item->docketFieldInfo->docket_field_category_id!=7)
                                                <tr>
                                 ...
                                 @else
                                  <td>
                                    <p>{{ $item->label }}</p>
                                      <ul class="images-3">
                                        @foreach($item->sentDocketImageValue as $signature)
                                          <li>  <a href="{{ AmazoneBucket::url() }}{{ $signature->value }}" target="_blank">
                                                  <img src="{{ AmazoneBucket::url() }}{{ $signature->value }}">
                                                   </a>
                                                   </li>
                                        @endforeach
                                                        ...
                                                        <table class="unit-per">
                                                            <tr>
                                                                @foreach($item->sentDocketUnitRateValue as $row)
                                                                    <td style="font-size:12px;">{{ $row->docketUnitRateInfo->label }}:{{ $item->sentDocketUnitRateValue->first()->value }}</td>
                                                                @endforeach
                                                            </tr>
                                                            <tr>
                                                                <td style="font-size:12px;" class="bold-d " colspan="2">Total </td>
                                                            </tr>
                                                        </table>
                                                    </td>
