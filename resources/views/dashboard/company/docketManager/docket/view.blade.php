...
@section('content')
    ...
    <div class="row" style="padding-top: 15px;background: #fff;margin: 0px;min-height: 400px;">
            ...
            <div id="printContainer">
                <div class="row invoice-info">
                    ...
                    <div class="col-md-12 invoice-col" style="    width: 100%;">
                        ...
                        <div class="pull-right" style="text-align:left;width:200px;">
                            <div style="width:100%">
                                <b style="word-break: break-word;">{{ @$sentDocket->template_title}}</b><br/>
                                                        ...
                                                        <table  class="table table-striped" width="100%">
                                                            ...
                                                            <tr>
                                                                @foreach($row->sentDocketFieldGridLabels as $gridFieldLabels)
                                                                    @if((!$gridFieldLabels->docketFieldGrid->is_hidden && $sentDocket->sender_company_id != Session::get('company_id')) || $sentDocket->sender_company_id==Session::get('company_id'))
                                                            ...
                                                            <tfooter>
                                                                <tr>
                                                                    @foreach($row->sentDocketFieldGridLabels as $gridFieldLsa)
                                                                        @if((!$gridFieldLsa->docketFieldGrid->is_hidden && $sentDocket->sender_company_id != Session::get('company_id')) || $sentDocket->sender_company_id==Session::get('company_id'))
                                                        ...
                                                        @else
                                                            @if($yesno['label_value'][$row->value]['label_type']==1)
                                                                <div style="width:50%; float:right;   padding-left: 8px;">
                                                                    <img style="width: 20px; height:20px; background-color:{{ $yesno['label_value'][$row->value]['colour']}}; border-radius:20px;padding:4px;" src="{{ AmazoneBucket::url() }}{{ $yesno['label_value'][$row->value]['label']}}">
                                                                </div>
                                                            @else
        ...
        <!--                        <a href="{{ url('dashboard/company/docketBookManager/docket/view/'.$sentDocket->id.'/approve') }}" class="btn btn-raised btn-xs  btn-success pull-right" id="addNew"><i class="fa fa-check"></i> Approve</a>-->
            <!--                    @else-->
                        ...
                        @if($sentDocket->status==0)
                            @if($sentDocket->sentDocketRecipientApproval)
                                @if(in_array(Auth::user()->id,$sentDocket->sentDocketRecipientApproval->pluck('user_id')->toArray()))
                                    ...
                                    @if(App\SentDocketRecipientApproval::where('sent_docket_id',$sentDocket->id)->where('status',1)->where('user_id',Auth::user()->id)->count()==1)
                                        <a href="javascript:void(0);" class="btn btn-raised btn-xs  btn-success pull-right" id="addNew"><i class="fa fa-check"></i> Approved</a>

                                    @elseif(App\SentDocketRecipientApproval::where('sent_docket_id',$sentDocket->id)->where('status',3)->where('user_id',Auth::user()->id)->count()==1)
            ...
            <script>
                ...
                $(document).on('click','#submitRejectDocket',function () {
                    ...
                    $.ajax({
                        ...
                        url: "{{url('dashboard/company/docketBookManager/docket/reject')}}",
