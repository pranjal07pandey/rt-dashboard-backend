...
@section('content')
    ...
    <div class="row" style="padding-top: 15px;background: #fff;margin: 0px;border: 1px solid #ddd;min-height: 400px;">
        ...
        <div class="col-md-8">
            <h3  class="pull-left" style="font-size: 20px; margin: 15px 0px 10px;font-weight: 500;display:inline-block"> <i class="fa fa-usd"></i>&nbsp;Billing History</h3>
            ...
            <table class="table" id="datatable">
                ...
                @foreach($invoices as $row)
                    <tr>
                        ...
                        <td>
                            @if($row->charge!="")
                                @if(\Stripe\Charge::retrieve($row->charge)->refunded)<span class="label label-warning">Refunded</span>@elseif($row->paid == true) <span class="label label-success">Paid</span> @else
                                    <span class="label label-danger">Un-paid</span> <br/>
                                    <span style="color:red;font-size: 12px;font-style: italic;">* {{ \Stripe\Charge::retrieve($row->charge)->failure_message }}</span>
                                @endif
                            @else
                        ...
                        <td>
                            <a href="{{ url('dashboard/company/profile/billingHistory/view/'.$row->id) }}"><i class="fa fa-eye"></i></a>&nbsp;&nbsp;
                            <button  class="btn btn-default btn-sm" onclick="location.href='{{url('dashboard/company/profile/billingHistory/downloadInvoice/'.$row->id)}}';" style="margin: 0px 0px 0px;"><i class="fa fa-file-pdf-o"></i></button>
                        </td>
                    </tr>
                @endforeach
                @if(count(@$invoices)==0)
...
@section('customScript')
    ...
    <script type="text/javascript">
        $(document).ready(function(){
            $('#datatable').DataTable({"ordering": false,"searching": false,"bLengthChange": false})
        });
    </script>
    @endif
@endsection