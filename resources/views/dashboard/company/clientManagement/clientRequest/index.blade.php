...
@section('content')
    ...
    <div class="boxContent" style="min-height: 500px;">
        <div class="boxHeader">
            <div class="pull-left">
                <strong>Clients Request</strong>
            </div>
            <div class="clearfix"></div>
        </div>

        <div class="boxBody">
            <table class="rtDataTable" id="clientRequestDatatable">
                <thead>
                    <tr>
                        <th>Company Name</th>
                        <th>ABN</th>
                ...
                <tbody>
                @if($company->clientRequest)
                    @foreach($company->clientRequest as $row)
                        @php
                        ...
                        <tr>
                            ...
                            <td>
                                <a href="{{ route('clients.request.action',array('accept',$row->id)) }}" class="btn btn-success  btn-xs btn-info" style="margin:0px;">
                                    <i class="fa fa-check"></i>&nbsp;&nbsp;Accept
                                </a>
                                <a href="{{ route('clients.request.action',array('reject',$row->id)) }}" class="btn btn-danger  btn-xs btn-info" style="margin:0px;">
...
@section('customScript')
    ...
    <script>
        $(document).ready(function() {
            $('#clientRequestDatatable').DataTable({ "order": [[ 4, "asc" ]] });
        });
    </script>
@endsection