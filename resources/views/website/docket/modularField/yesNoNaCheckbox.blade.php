        ...
        <div style="width:100%;margin:0;">
            ...
            @else
                @if($yesno['label_value'][$docketValue->value]['label_type']==1)
                    <div style="width:50%; float:right;   padding-left: 8px;"><img style="width: 20px; height:20px; background-color:{{ $yesno['label_value'][$docketValue->value]['colour']}}; border-radius:20px;padding:4px;" src="{{ AmazoneBucket::url() }}{{ $yesno['label_value'][$docketValue->value]['label'] }}"></div>
                @else
                    <div style="width:50%; float:right;    padding-left: 8px;">{{ $yesno['label_value'][$docketValue->value]['label']}}</div>
                @endif
            @endif
        </div>
        @if(count($docketValue->SentDocValYesNoValueInfo)==0)
        @else
            <table style="background: transparent; width: 100%;" class="table table-striped">
                ...
                <tbody >
                @foreach($docketValue->SentDocValYesNoValueInfo as $items)
